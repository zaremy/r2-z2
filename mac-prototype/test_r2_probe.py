...
class FakeClient:
    ...
    Responses are delivered through the real `_on_notify`, so the byte
    reassembly and the threading hand-off are under test too, not stubbed."""
...
class TestEventCapture(unittest.TestCase):
    ...
    def test_drain_empties_the_ring(self):
        ...
        self.assertEqual(r2.drain_events(), {"events": [], "count": 0, "dropped": 0})
...
class TestMatchedResponsesStillResolve(unittest.TestCase):
    ...
    def test_response_to_a_cancelled_send_is_noted_as_a_duplicate(self):
        """A send() cancelled outright (not timed out) leaves its future in
        _waiters. The late reply must not try to resolve it."""
        async def body():
            r2 = make_r2()
            r2._loop = asyncio.get_running_loop()
        ...
        self.assertEqual(events[0]["note"], "duplicate response")

    def test_our_seq_can_never_collide_with_the_notify_seq(self):
        """`% 0xFF` yields 0..254. If that ever became `% 0x100`, seq 255 would
        make a real response indistinguishable from a notification."""
        r2 = make_r2()
        seen = set()
        for _ in range(600):
            seen.add(r2._seq)
            r2._seq = (r2._seq + 1) % 0xFF
        self.assertNotIn(P.NOTIFY_SEQ, seen)
        self.assertEqual(max(seen), 0xFF - 1)
...
class FakeR2:
    ...
    async def _cmd(self, name, *args):
        ...
        return P.Response(P.FLAG_IS_RESPONSE, 0x17, 0x00, 1, 0, b"")
...
class TestNotifyOp(unittest.TestCase):
    ...
    def test_says_animation_complete_has_no_enable(self):
        """Upstream has no setter for it (animatronic.py:43), so the op must
        not imply it turned it on."""
        resp, _ = handle({"op": "notify", "params": {"leg": True}}, "read")
        self.assertNotIn("animation", resp["data"].keys() - {"note"})
...
class TestHandlerBasics(unittest.TestCase):
    ...
    def test_stop_is_allowed_at_read(self):
        resp, r2 = handle({"op": "stop"}, "read")
        self.assertTrue(resp["ok"])
        self.assertIn(("stop_animation",), r2.calls)

    def test_motion_ops_still_refused_at_read(self):
        for op in ("dome", "animation"):
            resp, r2 = handle({"op": op, "params": {"id": 1, "delta": 5}}, "read")
            self.assertFalse(resp["ok"])
            self.assertEqual(r2.calls, [])
